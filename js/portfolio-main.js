...
const transactions = [];
// ── Derived values ─────────────────────────────────────────────────────────
function marketValue() { return sharesOwned * currentPrice; }
function totalValue() { return cash + marketValue(); }
function pnl() { return totalValue() - STARTING_WORTH; }
// ── DOM refs ───────────────────────────────────────────────────────────────
...
const elBuy = document.getElementById('btn-buy');
const elSell = document.getElementById('btn-sell');
const elQtyInput = document.getElementById('quantity-input');
// ── Formatting helpers ─────────────────────────────────────────────────────
function fmtMoney(n) {
...
function executeBuy() {
    ...
    if (cost > cash) {
        showError(`Not enough cash. Need ${fmtMoney(cost)}, have ${fmtMoney(cash)}.`);
        return;
    }
    cash -= cost;
    sharesOwned += qty;
...
function executeSell() {
    ...
    if (qty > sharesOwned) {
        showError(`Only own ${sharesOwned} share${sharesOwned === 1 ? '' : 's'}.`);
        return;
    }
