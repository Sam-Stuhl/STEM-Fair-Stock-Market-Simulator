...
// Define the structure for canvas regions
interface CanvasRegions {
    chartArea: {x: number, y: number, width: number, height: number};
    priceAxis: {x: number, y: number, width: number, height: number};
    dateAxis: {x: number, y: number, width: number, height: number};
}

// Calculate the three regions of the canvas
export function getCanvasRegions (canvas: HTMLCanvasElement): CanvasRegions {
...
export function detectCanvasRegion(
    canvas: HTMLCanvasElement,
    mouseX: number,
    mouseY: number
): 'chartArea' | 'priceAxis' | 'dateAxis' | null {
...
function drawPriceLabelsOnCanvas(
    ...
    priceLabels.forEach((price, index) => {
        ...
        if (index === priceLabels.length - 1) return;
...
function drawDateLabelsOnCanvas(
    ctx: CanvasRenderingContext2D,
    regions: CanvasRegions,
    dateLabels: Array<{date: string, candleNum: number}>,
    totalCandles: number
): void {
...
// --- Helpers ---
function _getMinMax(candles: Candle[], padding: number) : [number, number] {
    // find min and max
    let min = Infinity;
    let max = -Infinity;
...
function _getPriceLabels(candles: Candle[], interval: number, min: number, max: number) : number[] {
    let startingPrice = candles[0].open;
    let prices: number[] = [startingPrice];
...
function _getDateLabels(candles: Candle[], interval: number) {
    let dateLabels = []
